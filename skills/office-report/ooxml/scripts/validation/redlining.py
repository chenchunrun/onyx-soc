...
class RedliningValidator:
    ...
    def __init__(self, unpacked_dir, original_docx, verbose=False, author="Author"):
        self.unpacked_dir = Path(unpacked_dir)
        self.original_docx = Path(original_docx)
        self.verbose = verbose
        self.author = author  # 可配置的作者名，用于验证修订
        self.namespaces = {
    ...
    def validate(self):
        ...
        # First, check if there are any tracked changes by the configured author to validate
        try:
            ...
            # Check for w:del or w:ins tags authored by the configured author
            del_elements = root.findall(".//w:del", self.namespaces)
            ins_elements = root.findall(".//w:ins", self.namespaces)

            # Filter to only include changes by the configured author
            author_del_elements = [
                elem
    ...
    def _remove_author_tracked_changes(self, root):
        """Remove tracked changes authored by the configured author from the XML root."""
