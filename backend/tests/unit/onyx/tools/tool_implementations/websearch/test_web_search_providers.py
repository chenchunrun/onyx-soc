...
def test_provider_requires_api_key() -> None:
    ...
    assert provider_requires_api_key(WebSearchProviderType.BIGMODEL) is True
...
def test_build_bigmodel_provider_requires_api_key() -> None:
    """Test that BigModel provider requires an API key."""
    with pytest.raises(ValueError, match="API key is required"):
        build_search_provider_from_config(
            provider_type=WebSearchProviderType.BIGMODEL,
            api_key=None,
            config={},
        )


def test_build_bigmodel_provider_with_default_config() -> None:
    """Test that BigModel provider can be built with just an API key."""
    ...
    assert isinstance(provider, BigModelClient)
    # DEFAULT_MAX_RESULTS = 20
    assert provider._num_results == 20  # noqa: SLF001
    assert provider._content_size == "medium"  # noqa: SLF001


def test_build_bigmodel_provider_respects_num_results() -> None:
    """Test that BigModel provider respects num_results from config."""
    from onyx.tools.tool_implementations.web_search.clients.bigmodel_client import (
        BigModelClient,
    )
    ...
    provider = build_search_provider_from_config(
        provider_type=WebSearchProviderType.BIGMODEL,
        api_key="test-api-key",
        config={"num_results": "20"},
    )
    assert isinstance(provider, BigModelClient)
    assert provider._num_results == 20  # noqa: SLF001


def test_build_bigmodel_provider_clamps_num_results() -> None:
    """Test that BigModel provider clamps num_results to valid range [1, 50]."""
    from onyx.tools.tool_implementations.web_search.clients.bigmodel_client import (
        BigModelClient,
    )

    # Too high should be clamped to 50
    provider = build_search_provider_from_config(
        provider_type=WebSearchProviderType.BIGMODEL,
        api_key="test-api-key",
        config={"num_results": "100"},
    )
    assert isinstance(provider, BigModelClient)
    assert provider._num_results == 50  # noqa: SLF001

    # Too low should be clamped to 1
    provider = build_search_provider_from_config(
        provider_type=WebSearchProviderType.BIGMODEL,
        api_key="test-api-key",
    ...
    assert isinstance(provider, BigModelClient)
    assert provider._num_results == 1  # noqa: SLF001
