...
# BigModel integration tests
pytestmark_bigmodel = pytest.mark.skipif(
    not os.environ.get("BIGMODEL_API_KEY"),
    reason="BIGMODEL_API_KEY not set; live web search tests require real credentials",
)


def _activate_bigmodel_provider(admin_user: DATestUser) -> int:
    response = requests.post(
        f"{API_SERVER_URL}/admin/web-search/search-providers",
        json={
            "id": None,
            "name": "integration-bigmodel-provider",
            "provider_type": WebSearchProviderType.BIGMODEL.value,
            "config": {},
    ...
    provider = response.json()
    assert provider["provider_type"] == WebSearchProviderType.BIGMODEL.value
    assert provider["is_active"] is True
    assert provider["has_api_key"] is True
...
@pytestmark_bigmodel
def test_web_search_endpoints_with_bigmodel(
    reset: None,  # noqa: ARG001
    admin_user: DATestUser,
) -> None:
    """Test web search endpoints using BigModel as the provider."""
    provider_id = _activate_bigmodel_provider(admin_user)
    assert isinstance(provider_id, int)

    search_request = {"queries": ["网络安全 漏洞"], "max_results": 3}
    ...
    assert lite_data["provider_type"] == WebSearchProviderType.BIGMODEL.value
    assert lite_data["results"], "Expected web search results from BigModel"
    ...
    assert combined_data["search_provider_type"] == WebSearchProviderType.BIGMODEL.value
    assert (
        combined_data["content_provider_type"]
        == WebContentProviderType.ONYX_WEB_CRAWLER.value
    )
