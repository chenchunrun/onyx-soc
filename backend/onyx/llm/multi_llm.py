...
# Known API bases -> environment variable names for API key override.
# When a litellm_proxy provider uses one of these API bases, the corresponding
# env var is checked as a fallback for the API key.
_KNOWN_LITELLM_API_KEYS: dict[str, str] = {
    "bigmodel.cn": "BIGMODEL_API_KEY",
    "api.moonshot.cn": "KIMI_API_KEY",
}
    ...
    def _completion(
        ...
        try:
            ...
            if "api_key" not in passthrough_kwargs:
                ...
                # Allow env var override for litellm_proxy providers with known API bases.
                # This lets users configure API keys via environment variables instead of
                # the database, which is useful for providers like BigModel and Kimi.
                if (
                    api_key is None
                    and self._model_provider == LlmProviderNames.LITELLM_PROXY
                    and self._api_base
                ):
                    api_key = _get_env_api_key_for_api_base(self._api_base)
                passthrough_kwargs["api_key"] = api_key
