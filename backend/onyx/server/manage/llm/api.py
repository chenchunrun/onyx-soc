...
def get_litellm_available_models(
    ...
    If `models` is provided, skip the /v1/models discovery call and use the
    manually specified model names instead. This is needed for providers like
    BigModel Code Planning that don't expose a /v1/models endpoint.
    """
    # If models are manually specified, skip discovery and return them directly
    if request.models:
        sorted_results = sorted(
            [
        ...
        # Sync manually specified models to DB if provider_name is specified
        if request.provider_name:
            _sync_fetched_models(
                db_session=db_session,
                provider_name=request.provider_name,
